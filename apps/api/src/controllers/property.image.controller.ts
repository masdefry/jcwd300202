...
export const getPropertyImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(propertyImagesByProperty.length < 0) throw { msg: 'Property images not found!', status: 406}
...
export const createPropertyImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(!createdPropertyImages) throw { msg: 'Create property images failed!', status: 406 }
...
export const deletePropertyImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        const { id, role } = req.body
        ...
        if(!getPropertyImage?.propertyDetail.property?.id || getPropertyImage?.propertyDetail.property?.deletedAt) throw { msg: 'Property not found!', status: 406 }
        if(getPropertyImage?.propertyDetail.property?.tenantId !== id) throw { msg: 'Actions not permitted!', status: 406 }
        

        if(!getPropertyImage?.id) throw { msg: 'Property image not found!', status: 406 }
        ...
        await prisma.$transaction(async(tx) => {
            try {
                ...
                fs.rmSync(`${getPropertyImage?.directory}/${getPropertyImage?.filename}.${getPropertyImage?.fileExtension}`)
            } catch (error) {
