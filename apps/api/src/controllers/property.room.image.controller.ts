...
export const getPropertyRoomImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(propertyRoomImagesByProperty.length < 0) throw { msg: 'Room images not found!', status: 406}
...
export const createPropertyRoomImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(!isPropertyRoomTypeExist?.property?.id || isPropertyRoomTypeExist?.property?.deletedAt) throw { msg: 'Property room type not found!', status: 406 }
        if(isPropertyRoomTypeExist?.property?.tenantId !== id) throw { msg: 'Actions not permitted!', status: 406 }
        ...
        if(!createdPropertyRoomImages) throw { msg: 'Create room images failed!', status: 406 }
...
export const deletePropertyRoomImagesByProperty = async(req: Request, res: Response, next: NextFunction) => {
    try {
        const { id, role } = req.body
        ...
        if(!getPropertyRoomImage?.propertyRoomType?.id || getPropertyRoomImage?.propertyRoomType?.deletedAt) throw { msg: 'Property room type not found!', status: 406 }
        if(getPropertyRoomImage?.propertyRoomType?.property?.tenantId !== id) throw { msg: 'Actions not permitted!', status: 406 }

        if(!getPropertyRoomImage?.propertyRoomType?.property?.id) throw { msg: 'Room image not found!', status: 406 }
        ...
        await prisma.$transaction(async(tx) => {
            try {
                ...
                fs.rmSync(`${getPropertyRoomImage?.directory}/${getPropertyRoomImage?.filename}.${getPropertyRoomImage?.fileExtension}`)
            } catch (error) {
