...
export const getCountriesService = async ({ countryName, limit }: Pick<ICountry, 'countryName'> & { limit: string | number }) => {
...
export const createCountryService = async ({ name, description, id, role, imagesUploaded }: Pick< ICountry, 'name' | 'imagesUploaded' | 'description'> & Pick<IUser, 'id' | 'role'>) => {
