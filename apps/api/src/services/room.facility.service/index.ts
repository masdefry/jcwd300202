...
export const getPropertyRoomFacilityService = async ({
  name,
...
export const createPropertyRoomFacilityService = async ({
  id,
  role,
  name,
  imagesUploaded,
}: Pick<IUser, 'id' | 'role'> &
  Pick<IPropertyRoomFacility, 'name'> & { imagesUploaded: any }) => {
  ...
  const createdPropertyRoomFacility = await prisma.propertyRoomFacility.create({
    data: {
      name,
