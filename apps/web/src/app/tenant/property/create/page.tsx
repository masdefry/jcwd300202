...
const CreatePropertyPage = () => {
  ...
  const [ showFormCreatePropertyType, setShowFormCreatePropertyType ] = useState<boolean>(false)
  ...
  const [ dataCreatePropertyType, setDataCreatePropertyType ] = useState({ name: '', description: '' })
  ...
  const handleInputPropertyType = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      <section className="mx-auto max-w-screen-2xl w-full h-full flex items-start gap-5 px-5 py-5">
        <div className='w-full'>
          ...
          onSubmit={(values) => {
            ...
            fd.append('propertyFacilitiesId', JSON.stringify(values?.propertyFacilitiesId.map((item) => ({ id: item }))))
            fd.append('propertyFacilitiesName', JSON.stringify(values?.propertyFacilitiesName.map((item) => ({ name: item }))))
                ...
                ({values, setFieldValue}) => (
                  ...
                  <section className="flex flex-col gap-5">
                    <hgroup className='flex flex-col mb-5'>
                      <h1 className='text-2xl font-bold text-gray-900'>Property General Information</h1>
                      <p className='text-sm font-medium text-gray-600'>General information is used to describe the entire property</p>
                    </hgroup>
                    <TextInput 
                    ...
                    <div className="grid items-center gap-1.5 w-full relative">
                      <Label htmlFor='propertyType' className='text-sm font-bold text-gray-900'>Property Type</Label>
                      <div className='flex items-center'> 
                        <input onChange={(e) => {
                          ...
                          if(e.target.value.length > 2) {
                            ...
                            const notFound = [{name: 'Minimum 3 characters', id: ''}]
                            setDataPropertyTypes(notFound)
                          }
                          }} name='propertyType' value={inputPropertyType} type="text" id='propertyType' placeholder={propertyType?.name ? '' : 'Hotel / Apartment'} 
                        ...
                        <button onClick={() => setShowFormCreatePropertyType(true)} disabled={Boolean(propertyType?.name)} className='disabled:bg-slate-300 disabled:opacity-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:scale-100 min-w-max text-white text-sm hover:opacity-75 transition duration-100 active:scale-95 font-bold rounded-r-full bg-gray-900 border-2 border-gray-900 p-5 py-1.5' type='button'>Add new</button>
                      </div>
                      {
                        (dataPropertyTypes && dataPropertyTypes.length > 0) && (
                        <ul id='property-type-list'  className='bg-white z-[51] absolute top-[65px] w-full flex flex-col rounded-md text-sm text-gray-800 border-2 border-gray-900 overflow-hidden'>
                          {
                            (dataPropertyTypes && dataPropertyTypes.length > 0) && (
                              dataPropertyTypes.map((item: any, index: number) => {
                                return (
                                  <li className={`px-5 py-2 ${(item?.id) ? 'hover:bg-gray-800 hover:text-white hover:cursor-pointer' : 'bg-gray-300 text-gray-600'} flex items-center gap-1.5 active:opacity-80 transition duration-100`}
                                  onClick={() => {
                                    if(item?.id) {
                                      setPropertyType({ name: item?.name, id: item?.id })
                      ...
                      {
                        propertyType?.name && (
                          <div className='absolute top-[25px] border-2 border-slate-600 py-1 px-3 flex items-center gap-1.5 text-sm font-bold text-gray-800 rounded-full bg-white w-fit'><FaRegBuilding className='text-base text-gray-600'/>
                          {propertyType?.name}
                          <IoClose className='text-base ml-2 hover:cursor-pointer hover:opacity-60'
                          onClick={() => {
                        ...
                        showFormCreatePropertyType && (
                          <div className='fixed bg-black bg-opacity-20 backdrop-blur-sm w-full h-full z-[51] top-0 left-0 flex items-center justify-center'>
                            <div className='bg-white border border-slate-200 shadow-md p-5 rounded-md flex flex-col gap-7'>
                              <div className='flex items-center justify-end'><IoClose className='hover:opacity-75 hover:cursor-pointer text-gray-900 ' onClick={() => setShowFormCreatePropertyType(false)} /></div>
                              <hgroup className='flex flex-col mt-[-10px]'>
                                <h1 className='text-lg font-bold text-slate-800'>Add Property Type</h1>
                                <p className='text-sm font-light text-gray-500'>Can't Find Your Property Type? Build It Here!</p>
                              </hgroup>
                              <div className='flex flex-col gap-3'>
                                <div className='flex flex-col gap-1 '>
                                  <label className='text-sm font-bold text-black ml-5'>Name</label>
                                  <Field id='propertyTypeName' 
                                  onChange={(e: any) => {
                                  ...
                                  name='createPropertyTypeName' type='text' placeholder='Hotel / Apartment / Villa' className='placeholder-shown:text-sm placeholder-shown:text-slate-300 focus:outline-none text-sm font-medium text-gray-900 focus:ring-slate-600 border border-slate-300 rounded-full px-5 py-2' />
                                ...
                                <div className='flex flex-col gap-1 '>
                                  <label className='text-sm font-bold text-black ml-5'>Description</label>
                                  <Field as='textarea' 
                                  onChange={(e: any) => {
                                    setDataCreatePropertyType((state: any) => {
                                  ...
                                  }} id='createPropertyTypeDescription' name='propertyTypeDescription' type="text" placeholder='Highlight its main features and target audience.' className='placeholder-shown:text-sm placeholder-shown:text-slate-300 focus:outline-none text-sm font-medium text-gray-900 focus:ring-slate-600 border border-slate-300 rounded-3xl px-5 py-2 h-[150px]' />
                                  <ErrorMessage name='propertyTypeDescription' component={'div'} className='text-red-600 px-4 text-xs font-bold mt-[-10px] ml-5 bg-red-200 p-1 rounded-full'/>
                                </div>
                              </div>
                              <div className='flex items-center gap-2 justify-end'>
                                <button type='button' onClick={() => setShowFormCreatePropertyType(false)} className='px-5 hover:bg-slate-200 transition duration-100 active:scale-90 py-1.5 text-gray-700 text-sm font-bold rounded-full shadow-md border border-slate-100 '>Cancel</button>
                                <button type='button' disabled={isPendingCreatePropertyType} onClick={() => mutateCreatePropertyType()} className='disabled:bg-slate-300 disabled:text-white disabled:scale-100 disabled:opacity-100 px-5 hover:opacity-75 transition duration-100 active:scale-90 py-1.5 text-white text-sm font-bold rounded-full shadow-md border bg-gray-900 border-slate-100 '>Create</button>
                              </div>
                            </div>
                          </div>
                        )
                    ...
                    <section id='region' className='flex flex-col xl:flex-row gap-5 items-center'>
                      ...
                      <div className='flex items-center'> 
                        <input onChange={(e) => {
                          ...
                          }} name='city' value={inputCity} type="text" id='city' placeholder={city?.name ? '' : 'Jakarta / New York'} 
                          className="w-full py-1.5 border-b-2 border-slate-300 text-sm placeholder-shown:text-sm rounded-none focus:outline-none focus:border-blue-600"/>
                        <button onClick={() => {
                          ...
                          disabled={Boolean(city?.name)} className='disabled:bg-slate-300 disabled:opacity-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:scale-100 min-w-max text-white text-sm hover:opacity-75 transition duration-100 active:scale-95 font-bold rounded-r-full bg-gray-900 border-2 border-gray-900 p-5 py-1.5' type='button'>Add</button>
                      </div>
                      {
                        (dataCities && dataCities.length > 0) && (
                        <ul id='city-list'  className='bg-white z-[51] absolute top-[65px] w-full flex flex-col rounded-md text-sm text-gray-800 border-2 border-gray-900 overflow-hidden'>
                          {
                            (dataCities && dataCities.length > 0) && (
                      ...
                      <div className='flex items-center'> 
                        <input onChange={(e) => {
                          ...
                          }} name='country' value={inputCountry} type="text" id='country' placeholder={country?.name ? '' : 'Indonesia'} 
                          className="w-full py-1.5 border-b-2 border-slate-300 text-sm placeholder-shown:text-sm rounded-none focus:outline-none focus:border-blue-600"/>
                        <button onClick={() => {
                          ...
                          disabled={Boolean(country?.name)} className='disabled:bg-slate-300 disabled:opacity-100 disabled:cursor-not-allowed disabled:border-slate-300 disabled:scale-100 min-w-max text-white text-sm hover:opacity-75 transition duration-100 active:scale-95 font-bold rounded-r-full bg-gray-900 border-2 border-gray-900 p-5 py-1.5' type='button'>Add</button>
                      </div>
                      {
                        (dataCountries && dataCountries.length > 0) && (
                        <ul id='country-list'  className='bg-white z-[51] absolute top-[65px] w-full flex flex-col rounded-md text-sm text-gray-800 border-2 border-gray-900 overflow-hidden'>
                          {
                            (dataCountries && dataCountries.length > 0) && (
                  ...
                  <section className="flex flex-col gap-5">
                    <hgroup className='mb-5'>
                      <h1 className='text-2xl font-bold text-gray-900'>Detail Property Information</h1>
                      <p className='text-sm font-medium text-gray-600'>property detail is used to explain in more detail what the property has</p>
                    </hgroup>
                    <TextInput
                    ...
                    <section className='flex sm:flex-row flex-col sm:items-center justify-between bg-white shadow-md border border-gray-200 rounded-md p-3'>
                      <div className="grid items-center gap-1.5 w-full relative">
                        <label htmlFor='checkInStartTime' className='text-sm font-bold text-gray-900'>Check-In Start Time</label>
                        <Field name='checkInStartTime' type='time' id='checkInStartTime' onChange={(e: any) => {
                          setFieldValue('checkInStartTime', e.target.value)
                          setInputCheckInStartTime(e.target.value)
                          }} 
                          value={inputCheckInStartTime} className="hover:cursor-text hover:opacity-60 transition duration-100 w-fit rounded-md px-3 py-1.5 border-2 border-slate-300 text-sm placeholder-shown:text-sm focus:outline-none focus:border-blue-600"/>
                      </div>
                      <div className="grid items-center gap-1.5 w-full relative">
                        <label htmlFor='checkInEndTime' className='text-sm font-bold text-gray-900'>Check-In End Time (optional)</label>
                        <Field name='checkInEndTime' type='time' id='checkInEndTime' onChange={(e: any) => {
                          setFieldValue('checkInEndTime', e.target.value)
                          setInputCheckInEndTime(e.target.value)
                          }} 
                          value={inputCheckInEndTime} className="hover:cursor-text hover:opacity-60 transition duration-100 w-fit rounded-md px-3 py-1.5 border-2 border-slate-300 text-sm placeholder-shown:text-sm focus:outline-none focus:border-blue-600"/>
                      </div>
                    </section>
                    <section className='flex sm:flex-row flex-col sm:items-center justify-between bg-white shadow-md border border-gray-200 rounded-md p-3'>
                      <div className="grid items-center gap-1.5 w-full relative">
                        <label htmlFor='checkOutStartTime' className='text-sm font-bold text-gray-900'>Check-Out Start Time (optional)</label>
                        <Field name='checkOutStartTime' type='time' id='checkOutStartTime' onChange={(e: any) => {
                          setFieldValue('checkOutStartTime', e.target.value)
                          setInputCheckOutStartTime(e.target.value)
                          }} 
                          value={inputCheckOutStartTime} className="hover:cursor-text hover:opacity-60 transition duration-100 w-fit rounded-md px-3 py-1.5 border-2 border-slate-300 text-sm placeholder-shown:text-sm focus:outline-none focus:border-blue-600"/>
                      </div>
                      <div className="grid items-center gap-1.5 w-full relative">
                        <label htmlFor='checkOutEndTime' className='text-sm font-bold text-gray-900'>Check-Out End Time</label>
                        <Field name='checkOutEndTime' type='time' id='checkOutEndTime' onChange={(e: any) => {
                          setFieldValue('checkOutEndTime', e.target.value)
                          setInputCheckOutEndTime(e.target.value)
                          }} 
                          value={inputCheckOutEndTime} className="hover:cursor-text hover:opacity-60 transition duration-100 w-fit rounded-md px-3 py-1.5 border-2 border-slate-300 text-sm placeholder-shown:text-sm focus:outline-none focus:border-blue-600"/>
                      </div>
                    </section>
                    <TextAreaCustom 
                  ...
                  <section className="flex flex-col gap-5">
                    <hgroup className='mb-5'>
                      <h1 className='text-2xl font-bold text-gray-900'>Property Facilities</h1>
                      <p className='text-sm font-medium text-gray-600'>Property facilities will explain to customers what they will get while staying in this property</p>
                    </hgroup>
                          ...
                          dataPropertyFacilities?.map((item: any, index: number) => {
                            return(
                              <div key={index} className="flex items-center space-x-2">
                                <Checkbox id="terms" value={item?.id} name='propertyFacilitesId' className='scale-90' 
                                onCheckedChange={(e) => {
                                  if(e) {
                  ...
                  <section className="flex flex-col gap-5">
                    <hgroup className='mb-5'>
                      ...
                      <p className='text-sm font-medium text-gray-600'>Showcase Your Space: Stunning Property Images for Renters </p>
                    </hgroup>
                    <FieldArray name='propertyImages'>
                      {
                        ({ insert: insertFilePropertyImage, remove: removeFilePropertyImage }) => (
                                                        ...
                                                        <div className="flex items-center justify-center 2xl:col-[1/4] 2xl:row-[1/3] col-span-4 w-full h-[150px] md:h-[320px] 2xl:h-[420px] overflow-hidden border-2 border-gray-300 border-dashed rounded-lg">
                                                          {
                                                            Boolean(values?.propertyImages[0]?.['name']) ? (
                                                              ...
                                                              <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                                ...
                                                                <input type="file" className="hidden" name={`propertyImages[0]`}
                                                                onChange={(e: any) => {
                                                            ...
                                                            Boolean(values?.propertyImages[imageIdx + 1]?.['name']) ? (
                                                              <figure className='w-full h-full relative'>
                                                                <Image  
                                                                  ...
                                                                  />
                                                                <div className='hover:cursor-pointer text-lg absolute right-4 bottom-4 bg-white shadow-md text-red-600 hover:text-opacity-75 active:scale-90 transition duration-100 h-10 w-10 flex items-center justify-center rounded-2xl'>
                                                                  <FaRegTrashCan onClick={() => removeFilePropertyImage(imageIdx + 1)}/>
                                                                </div>
                                                              </figure>
                                                            ) : (
                                                              <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                                ...
                                                                <input type="file" className="hidden" name={`propertyImages[${imageIdx + 1}]`}
                                                                onChange={(e: any) => {
                                                                  if(e.currentTarget.files[0]) {
                                                                    insertFilePropertyImage(imageIdx + 1, e.currentTarget.files[0])
                                                                  }
                                                                }}/>
                                                              </label>
                                                            )
                                                            ...
                                                            Boolean(values?.propertyImages[imageIdx + 3]?.['name']) ? (
                                                              <figure className='w-full h-full relative'>
                                                                <Image  
                                                                  ...
                                                                  />
                                                                <div className='hover:cursor-pointer text-lg absolute right-4 bottom-4 bg-white shadow-md text-red-600 hover:text-opacity-75 active:scale-90 transition duration-100 h-10 w-10 flex items-center justify-center rounded-2xl'>
                                                                  <FaRegTrashCan onClick={() => removeFilePropertyImage(imageIdx + 3)}/>
                                                                </div>
                                                              </figure>
                                                            ) : (
                                                              <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                                ...
                                                                <input type="file" className="hidden" name={`propertyImages[${imageIdx + 3}]`}
                                                                onChange={(e: any) => {
                                                                  if(e.currentTarget.files[0]) {
                                                                    insertFilePropertyImage(imageIdx + 3, e.currentTarget.files[0])
                                                                  }
                                                                }}/>
                                                              </label>
                                                            )
                  ...
                  <section className="flex flex-col gap-5">
                    <hgroup>
                      ...
                      <p className='text-sm font-medium text-gray-600'>Room types are used to provide customers with choices for several types in one property</p>
                    </hgroup>
                    {
                          ...
                          <div className='w-full flex flex-col gap-5'>
                            ...
                            { 
                              values.propertyRoomTypes.map((item, index) => {
                                return (
                                  <div key={index} className='flex gap-5 items-center px-5 py-2 rounded-full border-2 border-slate-900 bg-white relative'>
                                  <h1 className='text-sm font-bold text-gray-800'>{values.propertyRoomTypes[index].name || `Room type ${index + 1}`}</h1>
                                  <CiTrash 
                                ...
                                return (
                                  <div className='w-full flex flex-col gap-3'>
                                    <hgroup className='text-xl font-bold'>
                                      <h1>{values.propertyRoomTypes[index].name || `Room type ${index + 1}`}</h1>
                                    </hgroup>
                                    <Tabs defaultValue="roomGeneralInfo" className="w-full">
                                      ...
                                      <TabsContent value="roomGeneralInfo">
                                        <Card>
                                          <CardHeader>
                                            <CardTitle className='text-lg font-bold text-gray-900'>Room General Information</CardTitle>
                                      ...
                                      <TabsContent value="roomDetailInfo">
                                        <Card>
                                          <CardHeader>
                                            <CardTitle className='text-lg font-bold text-gray-900'>Room Details</CardTitle>
                                            <CardDescription className='text-sm font-medium text-gray-600'>
                                      ...
                                      <TabsContent value="roomFacilities">
                                        <Card>
                                          <CardHeader>
                                            <CardTitle className='text-lg font-bold text-gray-900'>Room Facilities</CardTitle>
                                            <CardDescription className='text-sm font-medium text-gray-600'>
                                                    ...
                                                    dataRoomFacilities?.map((roomFacility: any, indexRoomFacility: number) => {
                                                      return(
                                                        <div key={indexRoomFacility} className="flex items-center space-x-2">
                                                          <Checkbox checked={(values.propertyRoomTypes[index].roomFacilities.findIndex((value: any) => value === roomFacility?.id) > -1)} id={roomFacility?.id} value={roomFacility?.id} name={`propertyRoomTypes.${index}.roomFacilities.${indexRoomFacility}`}
                                                          onCheckedChange={(e: boolean) => {
                                                            if(e) {
                                                             pushRoomFacility(roomFacility?.id)
                                                            } else {
                                                              const findIdx = values.propertyRoomTypes[index].roomFacilities.findIndex((value: any) => value === roomFacility?.id)
                                                              values.propertyRoomTypes[index].roomFacilities.splice(findIdx, 1)
                                                              setChangedCheckbox(state => !state)
                                                            }
                                                          }}
                                                           className='scale-90'
                                                            />
                                      ...
                                      <TabsContent value="roomImages">
                                        <Card>
                                          <CardHeader>
                                            <CardTitle className='text-lg font-bold text-gray-900'>Room Images</CardTitle>
                                            <CardDescription className='text-sm font-medium text-gray-600'>
                                            Room images will be displayed when customers are exploring your property detail
                                            </CardDescription>
                                          </CardHeader>
                                          <CardContent>
                                          <FieldArray name={`propertyRoomTypes[${index}].roomImages`}>
                                            {
                                            ...
                                            <section className='flex flex-col gap-5 mb-8 mt-3'>
                                              ...
                                              {
                                                Boolean(values?.propertyRoomTypes[index]?.roomImages[0]?.['name']) ? (
                                                  <figure className='w-full h-full relative'>
                                                    <Image  
                                                      src={URL.createObjectURL(values?.propertyRoomTypes[index]?.roomImages[0])}
                                                      width={1000}
                                                      height={1000}
                                                      alt=''
                                                      className='object-cover w-full h-full' 
                                                  ...
                                                  <label className="flex flex-col items-center justify-center w-full h-full  cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                    ...
                                                    <input type="file" className="hidden" name={`propertyRoomTypes[${index}].roomImages[0]`}
                                                    onChange={(e: any) => {
                                                      if(e.currentTarget.files[0]) {
                                              ...
                                              <section className='grid grid-cols-4 gap-5'>
                                                {
                                                  Array.from({length:4}).map((_, imageIdx: number) => {
                                                    ...
                                                    <div key={imageIdx} className="flex items-center justify-center md:col-span-2 2xl:col-span-1 col-span-4 w-full h-[150px] lg:h-[200px] overflow-hidden border-2 border-gray-300 border-dashed rounded-lg">
                                                      {
                                                        Boolean(values?.propertyRoomTypes[index]?.roomImages[imageIdx + 1]?.['name']) ? (
                                                          <figure className='w-full h-full relative'>
                                                            <Image  
                                                              src={URL.createObjectURL(values.propertyRoomTypes[index].roomImages[imageIdx + 1])}
                                                              width={1000}
                                                              height={1000}
                                                              alt=''
                                                              className='object-cover w-full h-full' 
                                                          ...
                                                          <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                                            ...
                                                            <input type="file" className="hidden" name={`propertyRoomTypes[${index}].roomImages[${imageIdx + 1}]`}
                                                            onChange={(e: any) => {
                                                              if(e.currentTarget.files[0]) {
                  ...
                  <button className='rounded-full py-3 flex items-center gap-1.5 justify-center w-full transition duration-100 bg-black hover:opacity-75 active:scale-95 text-white text-sm font-bold'>Create Property</button>
                </Form>
                )
              }
          </Formik>
