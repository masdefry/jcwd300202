    ...
    <div className='flex flex-col gap-3'>
        <div className={`${isPending ? 'skeleton' : 'duration-200 hover:cursor-pointer active:scale-95'} h-[160px] lg:h-[230px] overflow-hidden transition relative w-full flex items-end px-4 py-3 rounded-md`}>
            ...
            <hgroup className='lg:hidden absolute bottom-4 left-4 z-20 flex flex-col gap-1 items-start'>
              <h1 className={`${isPending ? 'skeleton text-transparent bg-slate-300 rounded-none' : 'text-white'} ${h1Size} h-[1em] font-bold md:text-xl text-lg`}>{city}</h1>
              <h6 className={`${isPending ? 'skeleton text-transparent bg-slate-300 rounded-none' : 'text-white'} text-sm h-[1em] font-light`}>{country}</h6>
            </hgroup>
          </div>
          <hgroup className='hidden lg:flex flex-col gap-1 items-start z-10'>
            <h1 className={`${isPending ? 'skeleton text-transparent bg-slate-300 w-1/6 rounded-none' : 'text-black'} ${h1Size} font-bold h-[1em]`}>{city}</h1>
            <h6 className={`${isPending ? 'skeleton text-transparent bg-slate-300 w-1/8 rounded-none' : 'text-gray-600'} text-xl h-[1em] font-light`}>{country}</h6>
          </hgroup>
    </div>
  )
}
