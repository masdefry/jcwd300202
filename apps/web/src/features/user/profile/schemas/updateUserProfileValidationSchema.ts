...
export const updateUserProfileValidationSchema = Yup.object().shape({
    email: Yup.string().email('Email address invalid!').required('Field must be filled!'),
    username: Yup.string().matches(/^[a-zA-Z0-9\s]*$/, 'No special characters allowed!').nullable(),
    gender: Yup.string().matches(/^[a-zA-Z0-9\s]*$/, 'No special characters allowed').nullable(),
    phoneNumber: Yup.string().matches(/^[a-zA-Z0-9\s]*$/, 'No special characters allowed').nullable(),
    cityName: Yup.string().matches(/^[a-zA-Z0-9\s]*$/, 'No special characters allowed').nullable(),
    countryName: Yup.string().matches(/^[a-zA-Z0-9\s]*$/, 'No special characters allowed').nullable(),
